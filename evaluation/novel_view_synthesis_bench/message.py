...
def gather_csv_and_write(input_root: str, output_file: str):
    ...
    for seq_csv_file in sorted(os.listdir(input_root)):
        if seq_csv_file.endswith(".csv"):
            ...
            if len(df) > 1:
                print(
                    f"Warning: {osp.join(input_root, seq_csv_file)} has more than one row, only the last row will be saved."
                )
                df = df.tail(1)
            seq_dfs.append(df)
    ...
    if len(seq_dfs) == 0:
        raise ValueError(
            f"No CSV files found in {input_root}. Returning an empty DataFrame."
        )
...
def format_matrix_str(matrix):
    ...
    rows = [f"        [{', '.join(num for num in row)}]" for row in formatted]
    return "    [\n" + ",\n".join(rows) + "\n    ]"


def save_list_of_matrices(
