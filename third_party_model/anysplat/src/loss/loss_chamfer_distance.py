...
class LossChamferDistance(Loss[LossChamferDistanceCfg, LossChamferDistanceCfgWrapper]):
    ...
    def forward(
        ...
        for b_idx in range(b):
            batch_pts, batch_conf, batch_gaussian = (
                pred_pts[b_idx],
                conf_mask[b_idx],
                gaussian_meas[b_idx][pts_mask[b_idx]],
            )
            batch_pts = batch_pts[batch_conf]
            batch_pts = batch_pts[
                torch.randperm(batch_pts.shape[0])[
                    : int(batch_pts.shape[0] * self.cfg.down_sample_ratio)
                ]
            ]
            batch_gaussian = batch_gaussian[
                torch.randperm(batch_gaussian.shape[0])[
                    : int(batch_gaussian.shape[0] * self.cfg.down_sample_ratio)
                ]
            ]
            cd_loss = chamfer_distance(
                batch_pts.unsqueeze(0), batch_gaussian.unsqueeze(0)
            )[0]
