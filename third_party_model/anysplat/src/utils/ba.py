...
from third_party_model.anysplat.src.utils.tensor_to_pycolmap import (
...
def predict_track(
    model, images, query_points, dtype=torch.bfloat16, use_tf32_for_track=True, iters=4
):
    """
...
def initialize_feature_extractors(
    max_query_num, det_thres, extractor_method="aliked", device="cuda"
):
    """
    ...
    for method in methods:
        ...
        if method == "aliked":
            ...
            aliked_extractor = ALIKED(
                max_num_keypoints=aliked_max_points, detection_threshold=det_thres
            )
            extractors["aliked"] = aliked_extractor.to(device).eval()
        elif method == "sp":
            sp_max_points = max_query_num // active_extractors
            sp_extractor = SuperPoint(
                max_num_keypoints=sp_max_points, detection_threshold=det_thres
            )
    ...
    with torch.no_grad():
        for extractor_name, extractor in extractors.items():
            ...
            if query_points_round is not None:
                query_points_round = torch.cat(
                    [query_points_round, extractor_points], dim=1
                )
            else:
...
def run_vggt_with_ba(
    ...
    with torch.no_grad(), torch.cuda.amp.autocast(enabled=False, dtype=torch.bfloat16):
        aggregated_tokens_list, patch_start_idx = model.aggregator(
            images, intermediate_layer_idx=model.cfg.intermediate_layer_idx
        )
    with torch.cuda.amp.autocast(enabled=False):
    ...
    extractors = initialize_feature_extractors(
        max_query_num, det_thres, extractor_method, device
    )
    ...
    for query_index in query_frame_indexes:
        ...
        pred_track, pred_vis, pred_score = switch_tensor_order(
            [reorder_tracks, reorder_vis_score, reorder_conf_score],
            reorder_index,
            dim=0,
        )
