...
from third_party_model.anysplat.src.model.types import Gaussians
from third_party_model.anysplat.src.post_opt.datasets.colmap import Dataset, Parser
from third_party_model.anysplat.src.post_opt.datasets.traj import (
...
from third_party_model.anysplat.src.utils.image import process_image
from third_party_model.anysplat.src.post_opt.exporter import export_splats
from third_party_model.anysplat.src.post_opt.lib_bilagrid import (
    BilateralGrid,
    color_correct,
    slice,
...
from third_party_model.anysplat.src.post_opt.utils import (
    AppearanceOptModule,
    CameraOptModule,
    knn,
    rgb_to_sh,
...
from third_party_model.anysplat.src.post_opt.gsplat_viewer import (
    GsplatViewer,
...
def colorize_depth_maps(
    depth_map, min_depth=0.0, max_depth=1.0, cmap="Spectral", valid_mask=None
):
    """
    Colorize depth maps.
    """
...
class Config:
    ...
    eval_steps: List[int] = field(
        default_factory=lambda: [1, 1_000, 3_000, 7_000, 10_000]
    )
...
class Runner:
    ...
    def __init__(
        ...
        ctx_indices = [
            idx for idx, name in enumerate(image_names) if idx % cfg.test_every != 0
        ]
        tgt_indices = [
            idx for idx, name in enumerate(image_names) if idx % cfg.test_every == 0
        ]
        ...
        ctx_images = (
            torch.stack([images[i] for i in ctx_indices], dim=0).unsqueeze(0).to(device)
        )
        tgt_images = (
            torch.stack([images[i] for i in tgt_indices], dim=0).unsqueeze(0).to(device)
        )
        ...
        with (
            torch.no_grad(),
            torch.cuda.amp.autocast(enabled=False, dtype=torch.bfloat16),
        ):
        ...
        if isinstance(self.cfg.strategy, DefaultStrategy):
            self.strategy_state = self.cfg.strategy.initialize_state(scene_scale=1.0)
        ...
        for i, data in enumerate(valloader):
            ...
            if world_rank == 0:
                ...
                torchvision.utils.save_image(
                    ...
                    f"{self.render_dir}/gt_rgb_{stage}_step{step}_{i:04d}.png",
                )
                torchvision.utils.save_image(
                    colors.permute(0, 3, 1, 2),
                    f"{self.render_dir}/render_rgb_{stage}_step{step}_{i:04d}.png",
                )
